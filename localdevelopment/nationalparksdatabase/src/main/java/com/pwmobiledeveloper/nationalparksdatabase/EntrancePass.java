...
public class EntrancePass {
    ...
    public String getEntrancePassQuery(int parkId, int entrancePassId) {
        entrancePassId++;
        
        StringBuilder sb = new StringBuilder();
        sb.append("INSERT INTO entrancepass(id,park_id,"
                + "cost,description,title) VALUES (");
        //sb.append("(SELECT id FROM (SELECT COALESCE(MAX(id),0)+1 AS id FROM entrancepass) AS temp)");
        //sb.append("(SELECT id FROM (SELECT id AS id FROM park WHERE park_id = '" + parkId + "') AS temp)");
        sb.append(String.valueOf(entrancePassId).trim());
        sb.append(",");
